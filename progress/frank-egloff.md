...
Porting and deploying all Vertex contracts to Harmony. To address the mono-repo's reliance on the Vertex-typescript-sdk, I implemented a harmonyClient module designed to handle mock calls while facilitating the gradual porting of all API calls to the Harmony network. Despite encountering some deployment challenges, I continued in mocking the Vertex SDK methods to ensure its compatibility on the Harmony network.
